    ...
    ui(new Ui::annotateVideoEmotion)
{
    ...
    connect(ui->reference, &QPushButton::clicked, this, &annotateVideoEmotion::reference);
...
void annotateVideoEmotion::changeAnnotator()
{
    ...
    if (fileExists(d_sourcePath/fileName))
    {
        ...
        updateStats();
        d_current = std::distance(d_val.begin(), std::find(d_val.begin(), 
            d_val.end(), -1)); //set to first unrated clip, otherwise last clip
...
void annotateVideoEmotion::reference()
{
    ExampleDialog* dialog = new ExampleDialog(
        ui->valence->currentIndex() > -1 ? ui->valence->currentIndex() : 2, 
        ui->arousal->currentIndex() > -1 ? ui->arousal->currentIndex() : 2, 6);
    ...
    for (int idx = 1; idx < col + 1; ++idx)
    {
        ...
        if (d_ready)
        {
            if (idx - 1 == d_step[d_current])
                item->setBackground(QColor(251,176,59));
            else
...
{
    if (ui->catEmot->currentIndex() == -1)
    {
        QMessageBox messageBox;
        messageBox.critical(0, "Error", "Emotion label is not set");
        return;
    }
    saveResult();
...
{
    ui->valence->setCurrentIndex(d_val[d_current - 1]);
    ui->arousal->setCurrentIndex(d_aro[d_current - 1]);
    ui->catEmot->setCurrentIndex(d_lab[d_current - 1]);
}
...
void annotateVideoEmotion::saveResult()
{
    int val = ui->valence->currentIndex();
    int aro = ui->arousal->currentIndex();
    int lab = ui->catEmot->currentIndex();
    if (lab != -1)
    {
        d_val[d_current] = val;
        d_aro[d_current] = aro;
...
void annotateVideoEmotion::writeFile()
{
    try
    {
    ...
    catch (const exception e)
    {
        ...
        reply = QMessageBox::critical(this, "Error writing result", "An error occured in writing the result to the annotation file. Make sure it is not in use by another application. Do you want to try again?", QMessageBox::Yes|QMessageBox::No);
...
void annotateVideoEmotion::updateSubSliderLabels(int col)
{
    ...
    for (int idx = 0; idx < col; ++idx)
    {
        ...
        {
            if (idx == d_step[d_current])
                d_subSliderLabels[idx]->setStyleSheet("background-color:rgb(251,176,59); font-weight:bold;"); //
            else
...
{
    (d_val[d_current] == -1) ? ui->valence->setCurrentIndex(-1) : ui->valence->setCurrentIndex(d_val[d_current]);
    (d_aro[d_current] == -1) ? ui->arousal->setCurrentIndex(-1) : ui->arousal->setCurrentIndex(d_aro[d_current]);
    (d_lab[d_current] == -1) ? ui->catEmot->setCurrentIndex(-1) : ui->catEmot->setCurrentIndex(d_lab[d_current]);
}
...
void annotateVideoEmotion::setState()
{
    if (d_ready)
    {
        ...
        //combobox
        ui->valence->setEnabled(true);
        ui->arousal->setEnabled(true);
        ui->catEmot->setEnabled(true);
    ...
    else 
    {
        ...
        //combobox
        ui->valence->setEnabled(false);
        ui->arousal->setEnabled(false);
        ui->catEmot->setEnabled(false);
